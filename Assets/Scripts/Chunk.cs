...
public class Chunk : MonoBehaviour {
	...
	private void SpawnEnemies() {
		...
		for (int i = 0; i < enemyCount; i++) {
			...
			GameObject enemy = Instantiate(PrefabManager.Instance.Enemy, transform.position + offset, Quaternion.identity, EnemyManager.Instance.transform);
			EnemyManager.Instance.Enemies.Add(enemy.GetComponent<Enemy>());
		}
	}
	...
	private void SetChunks(Chunk topChunk, Chunk topRightChunk, Chunk rightChunk, Chunk btmRightChunk, Chunk btmChunk, Chunk btmLeftChunk, Chunk leftChunk, Chunk topLeftChunk) {
		top = topChunk;
	...
	private void SpawnTop() {
		GameObject chunk = Instantiate(chunkManager.ChunkPrefab, transform.position + chunkManager.VerStep, Quaternion.identity, chunkManager.transform);
		top = chunk.GetComponent<Chunk>();
		UpdateAllNeighbours();
	}

	private void SpawnTopRight() {
		GameObject chunk = Instantiate(chunkManager.ChunkPrefab, transform.position + chunkManager.HoriStep + chunkManager.VerStep, Quaternion.identity, chunkManager.transform);
		topRight = chunk.GetComponent<Chunk>();
		UpdateAllNeighbours();
	}

	private void SpawnRight() {
		GameObject chunk = Instantiate(chunkManager.ChunkPrefab, transform.position + chunkManager.HoriStep, Quaternion.identity, chunkManager.transform);
	...
	private void SpawnBtmLeft() {
		GameObject chunk = Instantiate(chunkManager.ChunkPrefab, transform.position - chunkManager.VerStep - chunkManager.HoriStep, Quaternion.identity, chunkManager.transform);
